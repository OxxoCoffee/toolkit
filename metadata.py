...
def metaextract(path):
    ...
    for imgfile in contfile:  # Itera sobre cada archivo en el directorio
        try:
            mimg = Image.open('../toolkit/Images/' + imgfile)  # Abre la imagen utilizando PIL
            exif_data = mimg.getexif()  # Obtiene los metadatos EXIF de la imagen
            rn = '../toolkit/Imagenes/' + imgfile + ' ' + 'Metadata' + '.txt'  # Construye el nombre del archivo de salida para guardar los metadatos
